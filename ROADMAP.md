...
> - **Branch protection на `main`** (SOC 2 §4): включён 20.09 и подтверждён через API — 1 обязательное ревью с code owner, 6 обязательных CI-проверок, force-push и удаление ветки запрещены. `enforce_admins` выключен (владелец может пушить напрямую); включить при втором разработчике.
>
> **Осталось (нельзя закрыть кодом):** первый прогон восстановления, ротация всех секретов, сбор отчётов/DPA вендоров, создание правил Sentry (токен), внешний uptime-монитор. И этап Liveness-проверки — см. §9 доки SOC 2.
